...
class DBarchive:
    ...
    exoPlanetArchiveURL = "https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+tic_id,hostname,st_rad,st_raderr1,st_raderr2,st_mass,st_masserr1,st_masserr2,st_teff,st_tefferr1,st_tefferr2,sy_dist,pl_letter,pl_orbper,pl_orbpererr1,pl_orbpererr2,pl_rade,pl_radeerr1,pl_radeerr2,pl_bmasse,pl_bmasseerr1,pl_bmasseerr2,pl_bmassprov,pl_orbincl,pl_orbinclerr1,pl_orbinclerr2,pl_orbeccen,pl_orbeccenerr1,pl_orbeccenerr2,pl_tranmid,pl_tranmiderr1,pl_tranmiderr2+from+ps+where+default_flag=1&format=csv"
    exoPlanetArchiveURL2 = "https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+tic_id,hostname,st_rad,st_raderr1,st_raderr2,st_mass,st_masserr1,st_masserr2,st_teff,st_tefferr1,st_tefferr2,sy_dist,pl_letter,pl_orbper,pl_orbpererr1,pl_orbpererr2,pl_rade,pl_radeerr1,pl_radeerr2,pl_bmasse,pl_bmasseerr1,pl_bmasseerr2,pl_bmassprov,pl_orbincl,pl_orbinclerr1,pl_orbinclerr2,pl_orbeccen,pl_orbeccenerr1,pl_orbeccenerr2,pl_tranmid,pl_tranmiderr1,pl_tranmiderr2+from+pscomppars&format=csv"
    ...
    insert_planet_template = Template("insert into planet values('$tname', '$pname', '$alt_name', $p, $pu, $pl, $pr, $pru, $prl, $pm, $pmu, $pml, '$pmt', $pi, $piu, $pil, $pe, $peu, $pel, $t0, $t0u, $t0l)")
    ...
    def create_database(self, dbname):
        ...
        try:
            ...
            dbcursor.execute("CREATE TABLE planet(tname text not null,pname text not null,alt_name text,period real not null,period_unc_upper real,period_unc_lower real,pradius real,pradius_unc_upper real,pradius_unc_lower real,pmass real,pmass_unc_upper real, pmass_unc_lower real,pmass_type text,pinc real,pinc_unc_upper real, pinc_unc_lower real,pecc real,pecc_unc_upper real,pecc_unc_lower real,t0 real,t0_unc_upper real,t0_unc_lower real,primary key(tname, pname),foreign key(tname) references target(tname));")
                ...
                for target in self.csvtargets.keys():
                    ...
                    if found_target:
                        ...
                        for p in planets:
                            ...
                            (csv_tname, csv_pname, csv_alt_name, csv_p, csv_pu, csv_pl, csv_pr, csv_pru, csv_prl, csv_pm, csv_pmu, csv_pml, csv_pmt, csv_pi, csv_piu, csv_pil, csv_pe, csv_peu, csv_pel, csv_t0, csv_t0u, cvs_t0l) = csv_planet_tuple
                            ...
                            if found_planet:
                                ...
                                if (csv_tname, csv_pname) not in self.excludeList: 
                                    insert_planet_stmt = self.insert_planet_template.substitute(tname=csv_tname, pname=csv_pname, alt_name=csv_alt_name, p=csv_p, pu=csv_pu, pl=csv_pl, pr=csv_pr, pru=csv_pru, prl=csv_prl, pm=csv_pm, pmu=csv_pmu, pml=csv_pml, pmt=csv_pmt, pi=csv_pi, piu=csv_piu, pil=csv_pil, pe=csv_pe, peu=csv_peu, pel=csv_pel, t0=csv_t0, t0u=csv_t0u, t0l=csv_t0l)
                                    dbcursor.execute(insert_planet_stmt)
                    else:
                        ...
                        if self.tic_id_in_db(dbcursor, csv_tic_id):
                            ...
                            for p in range(len(planets)):
                                ...
                                csv_planet_list = [ v if v != None else 'null' for v in csv_planet_list ]
                                csv_planet_tuple = tuple(csv_planet_list)
                                (csv_tname, csv_pname, csv_alt_name, csv_p, csv_pu, csv_pl, csv_pr, csv_pru, csv_prl, csv_pm, csv_pmu, csv_pml, csv_pmt, csv_pi, csv_piu, csv_pil, csv_pe, csv_peu, csv_pel, csv_t0, csv_t0u, csv_t0l) = csv_planet_tuple
                                if (csv_tname, csv_pname) not in self.excludeList:
        ...
        try:
            ...
            with open(csvname, newline='') as csvfile:
                ...
                for row in reader:
                    ...
                    (_tname, pname, anp, p, pu, pl, pr, pru, prl, pm, pmu, pml, pmt, pi, piu, pil, pe, peu, pel, t0, t0u, t0l) = tuple(planet_row)
                    if datasource == "exofop":
                        pname = pname[pname.find('.')+1:]
                    self.csvplanets[(tname, pname)] = (tname, pname, anp, p, pu, pl, pr, pru, prl, pm, pmu, pml, pmt, pi, piu, pil, pe, peu, pel, t0, t0u, t0l)
        ...
        try:
            ...
            with open(csvname, newline='') as csvfile:
                ...
                for row in reader:
                    ...
                    addl_csv.append((tname, tic_id, sr, sru, srl, sm, smu, sml, st, stu, stl, dist, pname, p, pu, pl, pr, pru, prl, pm, pmu, pml, pmt, pi, piu, pil, pe, peu, pel, t0, t0u, t0l))
        except Exception as e:
            print(str(e))
            traceback.print_exc()
    ...
    def filter_data(self, dbcursor):
        ...
        invalid_planets = []
        for (tname, pname) in self.csvplanets.keys():
            (tname, pname, alt_name, p, _pu, _pl, pr, _pru, _prl, pm, _pmu, _pml, _pmt, _pi, _piu, _pil, _pe, _peu, _pel, _t0, _t0u, _t0l) = self.csvplanets[(tname, pname)]
