...
class dynamite_plots:
    ...
    def __init__(self, Pk=[], P=[], PP=[], Rk=[], R=[], PR=[], ik=[], inc=[], Pi=[], ek=[], ecc=[], Pe=[], deltas=[], ratios=[], tdm=[], tdle=[], tdue=[], tpm=[], tple=[], tpue=[], targets=[], pers=[], rads=[], cfname="dynamite_config.txt"):
        ...
        if len(Pk) == 0:
            Pk, P, PP, Rk, R, PR, ik, inc, Pi, ek, ecc, Pe, deltas, ratios, tdm, tdle, tdue, tpm, tple, tpue, targets, pers, rads = self.read_saved_data()
        ...
        if self.config_parameters["plt_indpars"] == "True" and self.config_parameters["mode"] == "single":
            self.plot_ind_params(Pk, P, PP, pers, Rk, R, PR, ik, inc, Pi, ek, ecc, Pe)
    ...
    def read_saved_data(self):
        ...
        with np.load("saved_data.npz", allow_pickle=True) as fd:
            Pk, P, PP, Rk, R, PR, ik, inc, Pi, ek, ecc, Pe, deltas, ratios, tdm, tdle, tdue, tpm, tple, tpue, targets, pers, rads = fd["data"]
        
        return Pk, P, PP, Rk, R, PR, ik, inc, Pi, ek, ecc, Pe, deltas, ratios, tdm, tdle, tdue, tpm, tple, tpue, targets, pers, rads
    ...
    def plot_ind_params(self, Pk, P, PP, per, Rk, R, PR, ik, inc, Pi, ek, ecc, Pe,):
        ...
        if self.config_parameters["ind_P"] == "linear_zoom":
            ...
            ul = np.amax(n)*1.5
            xlim = [0.5 if min(np.hstack([p1, p11, p12, p2, p3])) > 0.5 else min(np.hstack([p1, p11, p12, p2, p3])) - 0.1, 7300 if system == "Inner Solar System" else 730]
            """
            xlim = [6, 23]
            ...
            plt.xscale("Log")
            xlim = [0.5 if min(np.hstack([p1, p11, p12, p2, p3])) > 0.5 else min(np.hstack([p1, p11, p12, p2, p3])) - 0.1, 7300 if system == "Inner Solar System" or system == "Proxima Centauri" else 730]
