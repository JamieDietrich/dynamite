...
from dynamite_plots_v2 import dynamite_plots
...
class dynamite:
    ...
    def __init__(self, merged_data=None):
        ...
        if self.config_parameters["saved"] == "False":
            if self.config_parameters["mode"] == "single":
                ...
                data = self.run_monte_carlo(R_star, Rse, M_star, Mse, target, target_name) + ([target[i][0] for i in range(len(target))], [target[i][1] for i in range(len(target))], [target[i][3] for i in range(len(target))])
                ...
                Pk, P, PP, Rk, R, PR, ik, il, Pinc, ek, el, Pecc, deltas, ratios, tdm, tdle, tdue, tpm, tple, tpue, targets, starvs, pers, rads, eccs = datavars = self.ppr.create_processes("mt_mc", (targets_dict, targlist), -len(targlist), self.process_data)
        ...
        if self.config_parameters["plot"] == "True":
            if self.config_parameters["saved"] == "False" and self.config_parameters["mode"] == "single":
                Pk, P, PP, Rk, R, PR, ik, il, Pinc, ek, el, Pecc, deltas, ratios, tdm, tdle, tdue, tpm, tple, tpue, targets, starvs, pers, rads, eccs = data

            elif self.config_parameters["saved"] == "False" and self.config_parameters["mode"] != "single":
                Pk, P, PP, Rk, R, PR, ik, il, Pinc, ek, el, Pecc, deltas, ratios, tdm, tdle, tdue, tpm, tple, tpue, targets, starvs, pers, rads, eccs = datavars
    ...
    def mt_mc(self, targets_dict, targlist, i):
        ...
        try:
            inds = np.argsort([target[i][0] for i in range(len(target))])
    ...
    def run_monte_carlo(self, R_star, Rse, M_star, Mse, target, target_name):
        ...
        if self.config_parameters["period"] == "epos":
            PP, deltas, cdfP = self.epos_pers(p0, per, rad, ecc, P, R, cdfR, Pecc, cdfe, M_star, GMfp213)

        elif self.config_parameters["period"] == "syssim":
            PP, deltas, cdfP = self.syssim_pers(per, rad, ecc, P, R, cdfR, Pecc, cdfe, M_star, GMfp213)
        ...
        stable, val, tim, _Pk, _Rk, _ek, _ik = self.ppr.create_processes("mc_test", (P, R, inew, ib, il, el, cdfP, cdfR, cdfi, cdfe, per, rad, inc, ecc, GMfp213, R_star, M_star), -int(self.interation_list[self.node_number - 1]), self.process_mc_data)
        ...
        tdm, tdle, tdue, tpm, tpue, tple, target_values = self.write_bf_pred_file(GMfp213, Pk, Rk, ik, ek, per, rad, ecc, Pecc, target_name, star_values, (True if self.num_of_nodes == 1 else False))

        return Pk, P, PP, Rk, R, PR, ik, il, Pinc, ek, el, Pecc, deltas, ratios, tdm, tdle, tdue, tpm, tpue, tple, target_values, star_values


    def write_bf_pred_file(self, GMfp213, Pk, Rk, ik, ek, per, rad, ecc, Pecc, target_name, star_values, write):
        ...
        if self.config_parameters["period"] == "epos":
            PPi, _, _ = self.epos_pers(min(per), per, rad, ecc, Pis, R, Rk, Pecc, cdfe, M_star, GMfp213)

        elif self.config_parameters["period"] == "syssim":
            PPi, _, _ = self.syssim_pers(per, rad, ecc, Pis, R, Rk, Pecc, cdfe, M_star, GMfp213)
    ...
    def mc_test(self, P, R, inew, ib, il, el, cdfP, cdfR, cdfi, cdfe, per, rad, inc, ecc, GMfp213, R_star, M_star, k):
    ...
    def epos_pers(self, p0, per, rad, ecc, P, R, PR, Pecc, cdfe, M_star, GMfp213):
