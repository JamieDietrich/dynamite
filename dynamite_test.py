...
class dynamite:
    ...
    def __init__(self, run=True, merged_data=None):
        """Starts DYNAMITE by initializing variables"""
        ...
        if run:
            self.run_dynamite(merged_data)



    def run_dynamite(self, merged_data=None):
        """Runs DYNAMITE"""
...
#        else:
    ...
    def run_single(self, targsys, run=True, domp=True):
        ...
        if not run:
            if targets_dict == None or len(targets_dict) == 0:
                print(targsys, "not found in database, creating simulated systems")
                return None

            alt_name, R_star, Rse, M_star, Mse, target, target_name, limits = self.set_up(targets_dict, limits_dict, targsys)
            targ = [i[:-1] for i in target]
            target = sorted(targ, key=lambda x: x[0][0])
            print(targsys, "found in database, running DYNAMITE")
            return target
        ...
        data = self.run_monte_carlo(R_star, Rse, M_star, Mse, target, target_name, limits, domp) + ([target[i][0] for i in range(len(target))], [target[i][1] for i in range(len(target))], [target[i][2] for i in range(len(target))], [target[i][4] for i in range(len(target))])
    ...
    def run_plots(self, data, targsys):
        ...
        dynamite_plots(Pk, P, PP, Rk, R, PR, ik, il, Pinc, ek, el, Pecc, deltas, tdm, tdle, tdue, tpm, tple, tpue, targets, starvs, pers, rads, mass, eccs, targsys, self.cfname, None) #(self.ppr if (sys.platform != "darwin" and os.cpu_count() > 16) else None))



    def mt_mc(self, i, args):
        """Runs the Monte Carlo code on multiple threads"""

        targets_dict, limits_dict, targlist = args
    ...
    def mr_convert(self, meas, pred, force=None):
        ...
        # Currently only runs one M-R relationship but more could be added here in the future #
    ...
    def run_monte_carlo(self, R_star, Rse, M_star, Mse, target, target_name, limits, domp=True):
        ...
        if self.config_parameters["inclination"] == "rayleigh_iso":
            ...
            il, Pinc, cdfi, inew, ib = self.syssim_incs(inc, per, GMfp213, R_star, domp)
...
#        if sys.platform != "darwin" and os.cpu_count() > 16:
#            PPi, PRi, Ri, deltasi, stable, val, tim, _Pk, _Rk, _ek, _ik = self.ppr.create_processes("mc_test", -int(self.interation_list[self.node_number - 1]), (P, PP, cdfP, per, deltas, R, PR, cdfR, inew, ib, il, cdfi, el, Pecc, em, cdfe, per_tup, rad_tup, mas_tup, inc_tup, ecc_tup, GMfp213, R_star, M_star, limits, indq), self.process_mc_data)

#        else:
        results = []
        res = self.run_new_mp(self.mc_test, np.arange(self.interations), (P, PP, cdfP, per, deltas, R, PR, cdfR, inew, ib, il, cdfi, el, Pecc, em, cdfe, per_tup, rad_tup, mas_tup, inc_tup, ecc_tup, GMfp213, R_star, M_star, limits, indq), domp)
    ...
    #def mc_test(self, P, PP, cdfP, per, deltas, R, PR, cdfR, inew, ib, il, cdfi, el, Pecc, em, cdfe, per_tup, rad_tup, mas_tup, inc_tup, ecc_tup, GMfp213, R_star, M_star, limits, indq, k):
    ...
    def mc_test(self, k, args):
        ...
        P, PP, cdfP, per, deltas, R, PR, cdfR, inew, ib, il, cdfi, el, Pecc, em, cdfe, per_tup, rad_tup, mas_tup, inc_tup, ecc_tup, GMfp213, R_star, M_star, limits, indq = args
        np.random.seed(self.seed_start + k + np.random.randint(100000))
        accept = "NO"
        itr = 0

        if sum(cdfP) == 0:
            return PP, PR, R, deltas, accept, 0, 0, 0, 0, 0, 0
    ...
    def create_pers(self, fP, per, mas, Ms, ecc, P, R, cdfR, il, Pinc, Pecc, em, cdfe, M_star, GMfp213):
        ...
        for i in range(len(P)):
            ...
            if len(Ms) == 0:
                ...
                fP[i] *= spst.lognorm.cdf(Dv[i], spst.norm.rvs(0.4, 0.02, 1), loc=0, scale=np.exp(spst.norm.rvs(1.97, 0.03, 1)))[0]
    
            else:
                for n in range(len(il)):
                    Ds[n] = max(0, aet * ((Ms[n, l] + m2)*(self.M_earth/(3*M_star*self.M_sun)))**(-1/3))
                    pdfP[n] = spst.lognorm.cdf(Ds[n], spst.norm.rvs(0.4, 0.02, 1), loc=0, scale=np.exp(spst.norm.rvs(1.97, 0.03, 1)))[0]
        ...
        if len(cdfP) > 0:
            ...
            if norm == 0 or np.isnan(norm):
                return np.zeros(len(fP)), np.zeros(len(fDu)), np.zeros(len(cdfP))

        else:
            return np.zeros(len(P)), np.zeros(len(fDu)), np.zeros(len(P))
        ...
        for k in range(len(incn)):
            ...
            for m in range(len(incn[k])):
                if self.config_parameters["inclination"] == "rayleigh_iso":
                    ...
                    test += spst.lognorm.pdf(abs(incn[k][m]-j), sigmas[len(inc)-1 if len(inc) <= len(sigmas) else -1], scale=1.1*((len(inc)+1)/5)**-1.73)
    ...
    def syssim_incs(self, inc, per, GMfp213, R_star, domp):
        ...
        if len(incq) == 0:
            ...
            fib = self.run_new_mp(self.inc_test, il, (inc, incn, sigmas), domp)
            mv = 0
            ib = 0
        ...
        inew = np.linspace(0, ilb, int(ilb*10) + 1)
        lgnrmi = spst.lognorm.pdf(inew, sigmas[len(inc)-1 if len(inc) <= len(sigmas) else -1], scale=1.1*((len(inc)+1)/5)**-1.73)
    ...
    def syssim_eccs(self, lecc):
        ...
        return el, spst.lognorm.pdf(el, sigmas[lecc if lecc < len(sigmas) else -1], scale=0.031*((lecc+1)/5)**-1.74), spst.lognorm.cdf(el, sigmas[lecc if lecc < len(sigmas) else -1], scale=0.031*((lecc+1)/5)**-1.74)
