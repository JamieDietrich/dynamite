...
class DBarchive:
    ...
    insert_planet_template = Template("insert into planet values('$tname', '$pname', $p, $pu, $pl, $pr, $pru, $prl, $pm, $pmu, $pml, $pi, $piu, $pil, $pe, $peu, $pel)")
    ...
    insert_dbscv_template = Template("insert into dbcsvmap values('$dbattr', '$datasource', $csvcolumn)")
    ...
    def create_database(self, dbname):
        ...
        try:
            ...
            dbcursor.execute("CREATE TABLE planet(tname text not null,pname text not null,period real not null,period_unc_upper real,period_unc_lower real,pradius real,pradius_unc_upper real,pradius_unc_lower real,pmass real,pmass_unc_upper real, pmass_unc_lower real,pinc real,pinc_unc_upper real, pinc_unc_lower real,pecc real,pecc_unc_upper real,pecc_unc_lower real,primary key(tname, pname),foreign key(tname) references target(tname));")
            ...
            for l in self.dbscv:
                dbcursor.execute(self.insert_dbscv_template.substitute(dbattr=l[0],datasource=l[1],csvcolumn="'"+l[2]+"'" if l[2] != None else 'null'))
            ...
            if self.empty_db:
                ...
                for (tname, pname) in self.csvplanets.keys():
                    if (tname, pname) not in self.excludeList:
                        ...
                        values = [ v if v != None else 'null' for v in values_list ]
                        (new_tname, new_pname, new_p, new_pu, new_pl, new_pr, new_pru, new_prl, new_pm, new_pmu, new_pml, new_pi, new_piu, new_pil, new_pe, new_peu, new_pel) = tuple(values)
                        insert_planet_stmt = self.insert_planet_template.substitute(tname=new_tname, pname=new_pname, p=new_p, pu=new_pu, pl=new_pl, pr=new_pr, pru=new_pru, prl=new_prl, pm=new_pm, pmu=new_pmu, pml=new_pml, pi=new_pi, piu=new_piu, pil=new_pil, pe=new_pe, peu=new_peu, pel=new_pel)        
                ...
                for target in self.csvtargets.keys():
                    ...
                    if found_target:
                        ...
                        for p in planets:
                            ...
                            (csv_tname, csv_pname, csv_p, csv_pu, csv_pl, csv_pr, csv_pru, csv_prl, csv_pm, csv_pmu, csv_pml, csv_pi, csv_piu, csv_pil, csv_pe, csv_peu, csv_pel) = csv_planet_tuple
                            ...
                            if found_planet:
                                ...
                                if (csv_tname, csv_pname) not in self.excludeList: 
                                    insert_planet_stmt = self.insert_planet_template.substitute(tname=csv_tname, pname=csv_pname, p=csv_p, pu=csv_pu, pl=csv_pl, pr=csv_pr, pru=csv_pru, prl=csv_prl, pm=csv_pm, pmu=csv_pmu, pml=csv_pml, pi=csv_pi, piu=csv_piu, pil=csv_pil, pe=csv_pe, peu=csv_peu, pel=csv_pel)
                                    dbcursor.execute(insert_planet_stmt)
                    else:
        ...
        try:
            ...
            with open(csvname, newline='') as csvfile:
                ...
                for row in reader:
                    ...
                    (_tname, pname, p, pu, pl, pr, pru, prl, pm, pmu, pml, pi, piu, pil, pe, peu, pel) = tuple(planet_row)
                    if datasource == "exofop":
                        pname = pname[pname.find('.')+1:]
                    self.csvplanets[(tname, pname)] = (tname, pname, p, pu, pl, pr, pru, prl, pm, pmu, pml, pi, piu, pil, pe, peu, pel)
    ...
    def filter_data(self, dbcursor):
        ...
        invalid_planets = []
        for (tname, pname) in self.csvplanets.keys():
            (tname, pname, p, _pu, _pl, pr, _pru, _prl, pm, _pmu, _pml, _pi, _piu, _pil, _pe, _peu, _pel) = self.csvplanets[(tname, pname)]
