...
class DBarchive:
    ...
    exoPlanetArchiveURL = "https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+hostname,st_rad,st_raderr1,st_raderr2,st_mass, st_masserr1,st_masserr2,st_teff,st_tefferr1,st_tefferr2,pl_letter,pl_orbper,pl_orbpererr1,pl_orbpererr2,pl_rade,pl_radeerr1,pl_radeerr2,pl_bmasse,pl_bmasseerr1,pl_bmasseerr2,pl_orbincl,pl_orbinclerr1,pl_orbinclerr2,pl_orbeccen,pl_orbeccenerr1,pl_orbeccenerr2+from+ps+where+default_flag=1&format=csv"
    ...
    insert_target_template = Template("insert into target values('$tname', $sr, $sru, $srl, $sm, $smu, $sml, $st, $stu, $stl, $rv)") 
    insert_planet_template = Template("insert into planet values('$tname', '$pname', $p, $pu, $pl, $pr, $pru, $prl, $pm, $pmu, $pml, $pi, $piu, $pil, $pe, $peu, $pel, $ttv)")
    ...
    def create_database(self, dbname):
        ...
        try:
            ...
            dbcursor.execute("CREATE TABLE planet(tname text not null,pname text not null,period real not null,period_unc_upper real,period_unc_lower real,pradius real,pradius_unc_upper real,pradius_unc_lower real,pmass real,pmass_unc_upper real, pmass_unc_lower real,pinc real,pinc_unc_upper real, pinc_unc_lower real,pecc real,pecc_unc_upper real,pecc_unc_lower real,ttv_limit real,primary key(tname, pname),foreign key(tname) references target(tname));")
            dbcursor.execute("CREATE TABLE target(tname text not null,sradius real not null,sradius_unc_upper real,sradius_unc_lower real,smass real not null,smass_unc_upper real,smass_unc_lower real,stemp real not null,stemp_unc_upper real,stemp_unc_lower real,rv_lim REAL,PRIMARY KEY(tname));")
            #dbcursor.execute("CREATE TABLE rvlimit(tname text not null,period real not null,semiamp_limit real not null,notes text not null,primary key (tname, period));")
            for l in self.dbcsv:
        ...
        try:
            if self.empty_db:
                for target in self.csvtargets.keys():
                    ...
                    (new_tname, new_sr, new_sru, new_srl, new_sm, new_smu, new_sml, new_st, new_stu, new_stl, new_rv) = tuple(values)
                    insert_target_stmt = self.insert_target_template.substitute(tname=new_tname, sr=new_sr, sru=new_sru, srl=new_srl, sm=new_sm, smu=new_smu, sml=new_sml, st=new_st, stu=new_stu, stl=new_stl,rv=new_rv)
                ...
                for (tname, pname) in self.csvplanets.keys():
                    if (tname, pname) not in self.excludeList:
                        ...
                        values = [ v if v != None else 'null' for v in values_list ]
                        (new_tname, new_pname, new_p, new_pu, new_pl, new_pr, new_pru, new_prl, new_pm, new_pmu, new_pml, new_pi, new_piu, new_pil, new_pe, new_peu, new_pel, new_ttv) = tuple(values)
                        insert_planet_stmt = self.insert_planet_template.substitute(tname=new_tname, pname=new_pname, p=new_p, pu=new_pu, pl=new_pl, pr=new_pr, pru=new_pru, prl=new_prl, pm=new_pm, pmu=new_pmu, pml=new_pml, pi=new_pi, piu=new_piu, pil=new_pil, pe=new_pe, peu=new_peu, pel=new_pel, ttv=new_ttv)
                ...
                for target in self.csvtargets.keys():
                    ...
                    if found_target:
                        ...
                        for p in planets:
                            ...
                            (csv_tname, csv_pname, csv_p, csv_pu, csv_pl, csv_pr, csv_pru, csv_prl, csv_pm, csv_pmu, csv_pml, csv_pi, csv_piu, csv_pil, csv_pe, csv_peu, csv_pel, csv_ttv) = csv_planet_tuple
                            ...
                            if found_planet:
                                ...
                                if (csv_tname, csv_pname) not in self.excludeList: 
                                    insert_planet_stmt = self.insert_planet_template.substitute(tname=csv_tname, pname=csv_pname, p=csv_p, pu=csv_pu, pl=csv_pl, pr=csv_pr, pru=csv_pru, prl=csv_prl, pm=csv_pm, pmu=csv_pmu, pml=csv_pml, pi=csv_pi, piu=csv_piu, pil=csv_pil, pe=csv_pe, peu=csv_peu, pel=csv_pel, ttv=csv_ttv)
                                    dbcursor.execute(insert_planet_stmt)
                    else:
                        (csv_tname, csv_sr, csv_sru, csv_srl, csv_sm, csv_smu, csv_sml, csv_st, csv_stu, csv_stl, csv_rv) = csv_target_tuple
                        insert_target_stmt = self.insert_target_template.substitute(tname=csv_tname, sr=csv_sr, sru=csv_sru, srl=csv_srl, sm=csv_sm, smu=csv_smu, sml=csv_sml, st=csv_st, stu=csv_stu, stl=csv_stl, rv=csv_rv)
                        dbcursor.execute(insert_target_stmt)
                        planets = [ p for (t,p) in self.csvplanets.keys() if t == target ]
                        for p in planets:
                            csv_planet_tuple = self.csvplanets[target, p]
        ...
        for dbattr in attrlist:
            if (dbattr, datasource) in self.dbcsvmap.keys() and self.dbcsvmap[(dbattr, datasource)] != None:
                ...
                if dbattr_value == '' or dbattr_value == None:
                    dbattr_value = None
                elif dbattr not in ['tname', 'pname']:
        ...
        try:
            ...
            with open(csvname, newline='') as csvfile:
                ...
                for row in reader:
                    ...
                    (tname, sr, sru, srl, sm, smu, sml, st, stu, stl, rv) = tuple(target_row)
                    if datasource == "exofop":
                        tname = "TOI-" + tname[:tname.find('.')]
                    if tname not in self.csvtargets:
                        self.csvtargets[tname] = (tname, sr, sru, srl, sm, smu, sml, st, stu, stl, rv)          
                    planet_row = self.get_row(row, datasource, self.dbattr_planet)
                    (_tname, pname, p, pu, pl, pr, pru, prl, pm, pmu, pml, pi, piu, pil, pe, peu, pel, ttv) = tuple(planet_row)
                    if datasource == "exofop":
                        pname = pname[pname.find('.')+1:]
                    self.csvplanets[(tname, pname)] = (tname, pname, p, pu, pl, pr, pru, prl, pm, pmu, pml, pi, piu, pil, pe, peu, pel, ttv)
    ...
    def filter_data(self, dbcursor):
        ...
        invalid_planets = []
        for (tname, pname) in self.csvplanets.keys():
            (tname, pname, p, _pu, _pl, pr, _pru, _prl, pm, _pmu, _pml, _pi, _piu, _pil, _pe, _peu, _pel, _ttv) = self.csvplanets[(tname, pname)]
