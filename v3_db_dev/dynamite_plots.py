...
class dynamite_plots:
    ...
    def __init__(self, Pk=[], P=[], PP=[], Rk=[], R=[], PR=[], ik=[], inc=[], Pi=[], ek=[], ecc=[], Pe=[], deltas=[], ratios=[], tdm=[], tdle=[], tdue=[], tpm=[], tple=[], tpue=[], targets=[], starvs=[], pers=[], rads=[], mass=[], eccs=[], cfname="dynamite_config.txt"):
        ...
        if len(Pk) == 0:
            Pk, P, PP, Rk, R, PR, ik, inc, Pi, ek, ecc, Pe, deltas, ratios, tdm, tdle, tdue, tpm, tple, tpue, targets, _starvs, pers, rads, _mass, _eccs = self.read_saved_data()
    ...
    def read_saved_data(self):
        ...
        with np.load("saved_data.npz", allow_pickle=True) as fd:
            Pk, P, PP, Rk, R, PR, ik, inc, Pi, ek, ecc, Pe, deltas, ratios, tdm, tdle, tdue, tpm, tple, tpue, targets, starvs, pers, rads, mass, eccs = fd["data"]
        
        return Pk, P, PP, Rk, R, PR, ik, inc, Pi, ek, ecc, Pe, deltas, ratios, tdm, tdle, tdue, tpm, tple, tpue, targets, starvs, pers, rads, mass, eccs
    ...
    def plot_P_R(self, Pk, P, Rk, R, targets, pers, rads):
        ...
        if self.config_parameters["saved"] == "False":
            ...
            np.savetxt("plot_fig_p" + self.config_parameters["mode"] + ".txt", plot_fig_p)
            np.savetxt("plot_fig_r" + self.config_parameters["mode"] + ".txt", plot_fig_r)

        elif self.config_parameters["saved"] == "True":
            plot_fig_p = np.loadtxt("plot_fig_p" + self.config_parameters["mode"] + ".txt")
            plot_fig_r = np.loadtxt("plot_fig_r" + self.config_parameters["mode"] + ".txt")
    ...
    def plot_ind_params(self, Pk, P, PP, Rk, R, PR, ik, inc, Pi, ek, ecc, Pe):
        ...
        def mr_appends(val_tup, m, r):
            ...
            return m, r
        """
        def mr_appends(pred, val):
            if self.config_parameters["mass_radius"] == "mrexo":
                ret = pfm(val, predict=pred, dataset='kepler')[0]
        ...
        for i in range(len(target)):
            if target[i][0] not in self.config_parameters["removed"]:
                if (system != "Inner Solar System" and target[i][3] != "?" and abs(math.cos(target[i][3]*math.pi/180)) * self.K3(target[i][0], Ms) / (Rs*self.R_sun) <= 1) or (system == "Inner Solar System" and target[i][1] < 1.5):
                    p1.append(target[i][0])
                    r1.append(target[i][1] if target[i][1] != "?" else mr_appends("radius", float(target[i][2])))
                    m1.append(target[i][2] if target[i][2] != "?" else mr_appends("mass", float(target[i][1])))
                    ...
                    r12.append(target[i][1] if target[i][1] != "?" else mr_appends("radius", float(target[i][2])))
                    m12.append(target[i][2] if target[i][2] != "?" else mr_appends("mass", float(target[i][1])))
        ...
        if len(self.config_parameters["additional"][0]) > 0:
            for i in self.config_parameters["additional"]:
                p11.append(i[0])
                r11.append(i[1] if i[1] != "?" else mr_appends("radius", i[2]))
                m11.append(i[2] if i[2] != "?" else mr_appends("mass", i[1]))
                i11.append(i[3])
                e11.append(i[4])
                l11.append(i[5])
        ...
        if len(self.config_parameters["unconfirmed"][0]) > 0:
            for i in self.config_parameters["unconfirmed"]:
                p3.append(i[0])
                r3.append(i[1] if i[1] != "?" else mr_appends("radius", i[2]))
                m3.append(i[2] if i[2] != "?" else mr_appends("mass", i[1]))
                i3.append(i[3])
                e3.append(i[4])
                l3.append(i[5])
            ...
            for i in range(len(self.config_parameters["removed"])):
                ...
                for j in target:
                    if j[0] == self.config_parameters["removed"][i]:
                        r2.append(j[1] if j[1] != "?" else mr_appends("radius", j[2]))
                        m2.append(j[2] if j[2] != "?" else mr_appends("mass", j[1]))
